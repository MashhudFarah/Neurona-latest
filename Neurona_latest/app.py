...
def submit_idea():
    ...
    if request.method == 'POST':
        ...
        funding_needed = float(request.form.get('funding_needed', 0))
        equity_offered = float(request.form.get('equity_offered', 0))
        stage = request.form['stage']
        contact_email = request.form['contact_email']

        # Founders and team_members: as JSON
        founders = json.dumps(request.form.getlist('founders[]'))
        team_members = json.dumps(request.form.getlist('team_members[]'))
        ...
        business_plan = save_file(request.files['business_plan'])
        product_image = save_file(request.files['product_image'])
        patent = save_file(request.files.get('patent'))

        # Handle multiple optional other files
        other_files_raw = request.files.getlist('other_files[]')
        other_files = [save_file(f) for f in other_files_raw if f and f.filename]
        other_files_json = json.dumps(other_files)
        ...
        if creator:
            ...
            c.execute('''
                INSERT INTO ideas (
                    creator_id, title, category, tags, summary,
                    problem_statement, solution, funding_needed, equity_offered,
                    stage, business_plan, product_image, patent, other_files,
                    founders, team_members, contact_email
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                creator_id, title, category, tags, summary,
                problem_statement, solution, funding_needed, equity_offered,
                stage, business_plan, product_image, patent, other_files_json,
                founders, team_members, contact_email
            ))
            conn.commit()
            conn.close()
...
@app.route('/explore')
def explore():
    if 'username' not in session or session['role'] != 'investor':
        flash('Only investors can access this page.', 'danger')
        return redirect(url_for('login'))
    ...
    if not user or user['verified'] != 1:
        flash('Only verified investors can access Explore.', 'warning')
        return redirect(url_for('investor_dashboard'))
...
def product_page(idea_id):
    ...
    return render_template('product.html', idea=idea)
